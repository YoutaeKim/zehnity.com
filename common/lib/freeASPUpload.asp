...
Dim adModeReadWrite : adModeReadWrite = 3
...
Class FreeASPUpload
	...
	Public Sub Save(path)
		...
		For Each fileItem In UploadedFiles.Items
		 ...
		 Next
	End Sub
	Public Function SaveFile_Info(path)
		...
		For Each fileItem In UploadedFiles.Items
			...
			SaveFile_Info = fileItem.Length & "/" &Mid(fileItem.FileName, InStrRev(fileItem.FileName, ".") + 1) & "/" & fileItem.FileName
		 Next
	End Function
	...
	Public Function DocumentSaveFile(path)
		Dim streamFile, fileItem, outFileName, strName, strExt, outLocalFileName
		...
		For Each fileItem In UploadedFiles.Items
			...
			strName = Mid(outFileName, 1, InstrRev(outFileName, ".") - 1) 
			strExt = Mid(outFileName, InstrRev(outFileName, ".") + 1)'확장자를 얻는다

			outLocalFileName = DocumentGetUniqueDate() & "." & strExt

			SaveImageName = outLocalFileName
			streamFile.SaveToFile path & SaveImageName, adSaveCreateOverWrite
			streamFile.close
			Set streamFile = Nothing
			fileItem.Path = path & SaveImageName
			DocumentSaveFile = SaveImageName
...
'N.B. Requires strSaveToPath variable to be available - and containing the path to save to
	...
	Response.Write "NewFullPath=" &NewFullPath
    Flag = False
    ...
    Loop
End Function
Function DocumentGetUniqueDate()
	Dim YYYY, MM, DD, DH, DM, SM, SS
	YYYY = Year(now)
	...
	Else
		SM = "0" & Second(now)
	End If
	SS = right("000" & (timer * 1000) Mod 1000, 3)

	DocumentGetUniqueDate = YYYY & MM & DD& "_" & DH & DM & SM & SS & DocumentGetUniqueDateRandom(2)
End Function 

Function DocumentGetUniqueDateRandom(Number)
	Dim documentautocpstr, documentautocpstrlen, documentautocpuniqNum
	documentautocpstr = "0123456789"
	documentautocpstrlen = Number
	documentautocpuniqNum = ""

	Randomize
	Dim documentautocpr, documentautocpi
	For documentautocpi = 1 To documentautocpstrlen
		documentautocpr = Int((10 - 1 + 1) * Rnd + 1)
		documentautocpuniqNum = documentautocpuniqNum & Mid(documentautocpstr, documentautocpr, 1)
	Next
	DocumentGetUniqueDateRandom = documentautocpuniqNum
